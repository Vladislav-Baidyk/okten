...
let content = document.createElement('div');
 for(let i = 0; i < coursesAndDurationArray.length; i++){
    ...
    let elementContent = document.createTextNode(`${coursesAndDurationArray[i].title}`);
    elementH1.classList.add("heading");
    elementH1.appendChild(elementContent);
    element.appendChild(elementH1);
    let elementDuration = document.createElement('p');
    elementDuration.classList.add("description");
    let elementDurationContent = document.createTextNode(`${coursesAndDurationArray[i].monthDuration}`);
